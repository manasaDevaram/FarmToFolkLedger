...
class PublicTraceCacheServiceTest {
    ...
    @Mock
    private FarmVerificationRepository farmVerificationRepository;

    @Mock
    private VerificationEvidenceRepository verificationEvidenceRepository;

    @Mock
    private FarmMediaRepository farmMediaRepository;
    ...
    void getStableDataFiltersPrivateEvidenceAndMedia() {
        ...
        when(qrCodeRepository.findByPublicTokenAndIsActiveTrue(publicToken)).thenReturn(Optional.of(qrCode));
        when(batchRepository.findById(batchId)).thenReturn(Optional.of(batch));
        when(farmerRepository.findById(farmerId)).thenReturn(Optional.of(farmer));
        when(farmRepository.findById(farmId)).thenReturn(Optional.of(farm));
        when(farmVerificationRepository.findFirstByFarmIdOrderByVerificationDateDesc(farmId)).thenReturn(Optional.of(verification));
        ...
        when(farmMediaRepository.findByFarmIdOrderByCreatedAtAsc(farmId)).thenReturn(List.of(publicMedia, privateMedia));
        ...
        assertEquals("https://example.com/public.jpg", stableData.verificationEvidence().getFirst().fileUrl());
        assertEquals(1, stableData.farmMedia().size());
        assertEquals("https://example.com/public-media.jpg", stableData.farmMedia().getFirst().mediaUrl());
    }
}
