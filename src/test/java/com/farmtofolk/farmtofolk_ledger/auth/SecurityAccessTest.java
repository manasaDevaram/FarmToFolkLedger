...
class SecurityAccessTest {
  ...
  void publicTraceDoesNotRequireAuthentication() throws Exception {
    mockMvc.perform(get("/api/public/trace/missing-token")).andExpect(status().isNotFound());
  }
  ...
  void fieldOfficerCannotUseAdminOrAdminOnlyBatchWrites() throws Exception {
    ...
    fieldOfficer.setPasswordHash(passwordEncoder.encode("password123"));
    fieldOfficer.setRole(UserRole.FIELD_OFFICER);
    fieldOfficer.setActive(true);
    fieldOfficer = userRepository.save(fieldOfficer);
    String authorization = "Bearer " + jwtService.generateToken(fieldOfficer);
    String batchId = "55e3153f-febb-4491-beb1-60a1d9ca0b1f";
