...
class GlobalExceptionHandlerTest {
  ...
  void resourceNotFoundReturns404Body() {
    ...
    ResponseEntity<ApiErrorResponse> response =
        handler.handleNotFound(new ResourceNotFoundException("Farmer not found"), request);
  ...
  void badRequestReturns400Body() {
    MockHttpServletRequest request =
        new MockHttpServletRequest("POST", "/api/batches/id/trace-events");

    ResponseEntity<ApiErrorResponse> response =
        handler.handleBadRequest(new BadRequestException("Invalid trace event type"), request);
