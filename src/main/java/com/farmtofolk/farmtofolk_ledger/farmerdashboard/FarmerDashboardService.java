...
public class FarmerDashboardService {
    ...
    public FarmerDashboardSummaryResponse getSummary(UUID requestedFarmerId) {
        ...
        UserResponse linkedUser = farmer.getUserId() == null ? null : userRepository.findById(farmer.getUserId())
                .map(UserResponse::from)
                .orElse(null);
        return new FarmerDashboardSummaryResponse(
                FarmerResponse.from(farmer), linkedUser,
    ...
    public List<FarmerDashboardBatchResponse> getBatches(UUID requestedFarmerId) {
        ...
        Map<UUID, Farm> farms = farmRepository.findAllById(
                batches.stream().map(Batch::getFarmId).collect(Collectors.toSet())
        ).stream().collect(Collectors.toMap(Farm::getId, Function.identity()));

        return batches.stream().map(batch -> {
            List<TraceEventResponse> events = traceEventRepository
                    .findByBatchIdOrderByEventTimeAsc(batch.getId()).stream()
                    .map(TraceEventResponse::from).toList();
            String latestTraceStatus = events.isEmpty() ? null : events.getLast().eventType();
            BatchProcurement procurement = procurementRepository.findByBatchId(batch.getId()).orElse(null);
            List<BatchSaleTransaction> sales = saleTransactionRepository
                    .findByBatchIdOrderBySoldAtAsc(batch.getId());
            BigDecimal totalQuantitySold = sumQuantitySold(sales);
            BigDecimal totalSaleAmount = sumSaleAmount(sales);
            BigDecimal quantityTaken = procurement == null ? BigDecimal.ZERO : procurement.getQuantityTaken();
            Farm farm = farms.get(batch.getFarmId());
            return new FarmerDashboardBatchResponse(
                    batch.getId(), batch.getBatchCode(), batch.getCropName(), batch.getVariety(),
                    farm == null ? null : farm.getFarmName(),
                    quantityTaken, batch.getUnit(), batch.getStatus(),
                    batch.getHarvestDate(), batch.getPackedDate(), batch.getBestBeforeDate(), latestTraceStatus,
                    procurement == null ? null : procurement.getFarmerPricePerUnit(),
                    procurement == null ? null : procurement.getFarmerAmountPayable(),
                    procurement == null ? null : procurement.getPaymentStatus(), totalQuantitySold,
                    quantityTaken.subtract(totalQuantitySold), totalSaleAmount,
    ...
    public FarmerDashboardBatchDetailResponse getBatchDetail(UUID batchId) {
        ...
        List<BatchSaleTransaction> sales = saleTransactionRepository.findByBatchIdOrderBySoldAtAsc(batchId);
        BigDecimal totalQuantitySold = sumQuantitySold(sales);
        BigDecimal quantityTaken = procurement == null ? BigDecimal.ZERO : procurement.getQuantityTaken();
        ...
        return new FarmerDashboardBatchDetailResponse(
                ...
                traceEventRepository.findByBatchIdOrderByEventTimeAsc(batchId).stream()
                        .map(TraceEventResponse::from).toList(),
                priceBreakdownRepository.findByBatchId(batchId).map(PriceBreakdownResponse::from).orElse(null),
                procurement == null ? null : BatchProcurementResponse.from(procurement),
                sales.stream().map(BatchSaleTransactionResponse::from).toList(),
                totalQuantitySold, quantityTaken.subtract(totalQuantitySold), sumSaleAmount(sales)
        );
    }

    private BigDecimal sumQuantitySold(List<BatchSaleTransaction> sales) {
        return sales.stream().map(BatchSaleTransaction::getQuantitySold)
                .reduce(BigDecimal.ZERO, BigDecimal::add);
    }

    private BigDecimal sumSaleAmount(List<BatchSaleTransaction> sales) {
        return sales.stream().map(BatchSaleTransaction::getSaleAmount)
                .reduce(BigDecimal.ZERO, BigDecimal::add);
    }
    ...
    private Farmer resolveDashboardFarmer(UUID requestedFarmerId) {
        ...
        return farmerRepository.findById(requestedFarmerId)
