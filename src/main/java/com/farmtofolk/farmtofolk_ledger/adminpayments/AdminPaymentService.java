...
public class AdminPaymentService {
    ...
    public List<FarmerPaymentSummaryResponse> getFarmerSummaries() {
        Map<UUID, List<AdminBatchPaymentResponse>> byFarmer = loadPayments().stream()
                .collect(Collectors.groupingBy(
                        AdminBatchPaymentResponse::farmerId,
                        LinkedHashMap::new,
                        Collectors.toList()
                ));

        return byFarmer.values().stream().map(payments -> {
            AdminBatchPaymentResponse first = payments.getFirst();
            BigDecimal totalPaid = totalForStatus(payments, "PAID");
            BigDecimal totalPending = totalForStatus(payments, "UNPAID");
            List<FarmerPaymentBatchResponse> batches = payments.stream()
                    .map(this::toFarmerBatch)
                    .toList();
            return new FarmerPaymentSummaryResponse(
                    first.farmerId(), first.farmerName(), first.farmerPhone(),
                    totalPaid.add(totalPending), totalPaid, totalPending,
                    payments.stream().filter(payment -> "UNPAID".equals(payment.paymentStatus())).count(),
                    batches
            );
    ...
    public List<AdminBatchPaymentResponse> getBatchPayments(
        ...
        return loadPayments().stream()
                .filter(payment -> normalizedStatus == null || normalizedStatus.equals(payment.paymentStatus()))
                .filter(payment -> farmerId == null || farmerId.equals(payment.farmerId()))
                .filter(payment -> normalizedCrop == null || normalizedCrop.isBlank()
    ...
    @Transactional
    public BatchProcurementResponse updatePaymentStatus(UUID procurementId, UpdatePaymentStatusRequest request) {
        BatchProcurement procurement = procurementRepository.findById(procurementId)
                .orElseThrow(() -> new ResourceNotFoundException("Procurement not found"));
        procurement.setPaymentStatus(normalizeStatus(request.paymentStatus()));
    ...
    private List<AdminBatchPaymentResponse> loadPayments() {
        ...
        Map<UUID, Batch> batches = batchRepository.findAllById(
                procurements.stream().map(BatchProcurement::getBatchId).collect(Collectors.toSet())
        ).stream().collect(Collectors.toMap(Batch::getId, Function.identity()));
        Map<UUID, Farmer> farmers = farmerRepository.findAllById(
        ...
        Map<UUID, Farm> farms = farmRepository.findAllById(
        ...
        for (BatchProcurement procurement : procurements) {
            ...
            payments.add(new AdminBatchPaymentResponse(
                    procurement.getId(), batch.getFarmerId(), farmer == null ? null : farmer.getName(),
                    farmer == null ? null : farmer.getPhone(), batch.getFarmId(),
                    farm == null ? null : farm.getFarmName(), batch.getId(), batch.getBatchCode(),
                    batch.getCropName(), procurement.getQuantityTaken(), batch.getUnit(),
    ...
    private BigDecimal totalForStatus(List<AdminBatchPaymentResponse> payments, String status) {
        return payments.stream()
                .filter(payment -> status.equals(payment.paymentStatus()))
    ...
    private FarmerPaymentBatchResponse toFarmerBatch(AdminBatchPaymentResponse payment) {
        return new FarmerPaymentBatchResponse(
                payment.batchId(), payment.batchCode(), payment.cropName(), payment.quantityTaken(),
                payment.unit(), payment.farmerPricePerUnit(), payment.farmerAmountPayable(),
                payment.paymentStatus(), payment.procuredAt()
        );
    }
