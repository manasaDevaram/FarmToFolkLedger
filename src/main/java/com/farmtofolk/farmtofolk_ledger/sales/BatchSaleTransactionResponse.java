...
public record BatchSaleTransactionResponse(
    ...
    public static BatchSaleTransactionResponse from(BatchSaleTransaction transaction) {
        return new BatchSaleTransactionResponse(
                transaction.getId(), transaction.getBatchId(), transaction.getQuantitySold(),
                transaction.getSalePricePerUnit(), transaction.getSaleAmount(), transaction.getCurrency(),
                transaction.getSoldAt(), transaction.getCreatedAt()
        );
    }
}
