...
public class BatchSaleTransactionService {
    ...
    public BatchSaleTransactionResponse create(UUID batchId, CreateBatchSaleTransactionRequest request) {
        findBatch(batchId);
        BatchProcurement procurement = procurementRepository.findByBatchIdForUpdate(batchId)
                .orElseThrow(() -> new BadRequestException("Procurement must be recorded before sale transactions"));

        BigDecimal soldAlready = saleTransactionRepository.findByBatchIdOrderBySoldAtAsc(batchId).stream()
    ...
    private void enforceFarmerOwnership(Batch batch) {
        ...
        boolean ownsBatch = farmerRepository.findByUserId(currentUserService.getCurrentUserId())
